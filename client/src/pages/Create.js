...
function CreatePage() {
  ...
  function validateName(name) {
    ...
    if (!name) {
      ...
      setValidateError((prevArray) => [
        ...prevArray,
        "Please enter a valid quiz name.",
      ]);
      return false;
    }
  ...
  function handleCreateQuiz(event) {
    ...
    console.log(
      `Quiz Information: Quiz Name: ${quizName}, author ${author}, thumbnail: ${thumbnail}, id: ${id}`
    );
        ...
        <Flex p={8} flex={12} align={"center"} justify={"center"}>
          <Stack spacing={6} w={"full"} maxW={"lg"}>
            ...
            <FormControl>
              ...
              <Button colorScheme="purple" onClick={handleCreateQuiz}>
                Next
              </Button>
              {validateError.map((error, index) => (
                <FormHelperText color={"red.600"} key={index}>
                  * {error}
                </FormHelperText>
              ))}
            </FormControl>
